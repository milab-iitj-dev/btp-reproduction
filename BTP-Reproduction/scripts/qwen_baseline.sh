...
echo "MODE: div_prune=$(grep -c 'def div_prune' $QMOD) (expect 0) | MODEL: qwen2.5-vl-7b"

for TASK in pope mme mmbench_en_dev textvqa_val gqa scienceqa_img seedbench; do
  ...
  accelerate launch --num_processes=1 -m lmms_eval \
    ...
    --log_samples --log_samples_suffix qwenbase \
    --output_path ./results/lmms_logs/ || echo "=== FAILED $TASK ==="
  echo "=== END $TASK $(date) ==="
done
