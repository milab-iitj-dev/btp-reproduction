#!/bin/bash
#SBATCH --job-name=qwen_btp_subset
...
cp ${QMOD}.BTP $QMOD
echo "MODE: div_prune=$(grep -c 'def div_prune' $QMOD) (expect 1) | qwen BTP subset 2000"

for TASK in seedbench textvqa_val; do
  echo "=== START $TASK $(date) ==="
  accelerate launch --num_processes=1 -m lmms_eval \
    --model qwen2_5_vl \
    --model_args pretrained="Qwen/Qwen2.5-VL-7B-Instruct",attn_implementation="flash_attention_2" \
    --tasks $TASK --batch_size 1 --limit 2000 \
    --log_samples --log_samples_suffix qwenbtp_2k \
