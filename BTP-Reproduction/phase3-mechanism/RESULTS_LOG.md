...
**CONVENTION, and a correction.** The two sweeps label layers differently, and an earlier
version of this section conflated them.

    Qwen sweep (E12) sets self.end_layer. The BTP source runs `layer_index += 1` BEFORE its
    branch check, so end_layer is ONE based: end_layer = E deletes the tokens before the
    E-th layer executes, leaving E-1 layers with access.

    InternVL sweep (E11) uses enumerate(), which is ZERO based, and also deletes before the
    named layer executes: IVL_WIPE_LAYER = L leaves exactly L layers with access.

The quantity that means something is the number of layers that still need the image, so
both are reported that way below. The Qwen figure is one lower than the raw end_layer value.

    InternVL2-2B   17.95 layers of 24  =  74.8% of the decoder
    Qwen2.5-VL-7B  22.48 layers of 28  =  80.3% of the decoder

An earlier entry gave Qwen as 83.9%, which used the raw end_layer value of 23.48 rather than
the count of layers with access. 80.3% is correct. The conclusions are unchanged and in fact
tidier: both models sit between 75 and 80 per cent rather than at 75 and 84.

BTP's released end_layer = 23 gives Qwen 22 layers of access against a threshold of 22.5,
so it still falls just below.
...
## 9. E13, the threshold measured with a portable tool (job 423959), VERIFIED

Both sweeps above edit model source, separately per architecture, which nobody else can
reuse. `measure_visual_depth.py` performs the same measurement with a PyTorch forward hook
and no source changes, so that it works on any Hugging Face vision language model. That
portability is only worth something if the hook reproduces what patching produced, so this
job ran it on Qwen2.5-VL-7B, where we already had the answer.

Tool run: 200 unique images, deduplicated by md5, own VQA scorer. Baseline 0.9100, which is
higher than E12's 0.8624 because the sample differs (200 unique images against lmms-eval's
first 500 rows, which contain duplicates). Compare the threshold, not the absolute scores.

| layers with access | source patch (E12) | forward hook (E13) |
|--------------------|--------------------|--------------------|
| 1                  |  8.3 %             |  9.9 %             |
| 7                  | 11.5 %             | 14.8 %             |
| 13                 | 15.7 %             | 17.0 %             |
...
(*) These two rows are the hook values at its own layers 23 and 24, which correspond to 23
and 24 layers with access, so they align with the source rows one line below. Read down the
aligned pairs: 27.2 against 27.5, and 74.8 against 75.8.

### Verdict

The instruments agree within a couple of points across the whole curve, on different sample
sets. The threshold is therefore a property of the model and not an artefact of how it was
measured. The tool is validated for Qwen2.5-VL.

### What the validation caught

The off-by-one described in section 8.8. The tool and the source disagreed by exactly one
layer until we traced it to the one-based `layer_index` in the BTP source. That is precisely
what a second instrument is for, and it is the reason our reported Qwen depth moved from
83.9 to 80.3 per cent.

### Bring-up, three failures worth recording

1. `position_embeddings` was read from kwargs alone, but Qwen passes it positionally, so the
   rotary tensors were never sliced. Fixed by binding arguments against the forward
   signature, which removes that whole class of bug.
2. Hooking only the deletion layer was not enough. Rotary embeddings are built once above
   the decoder loop and handed to every layer, so layers L+1 onward still held full-length
   cos and sin against a shortened hidden state. Every layer from L upward is now hooked:
   L deletes and records the keep index, the rest use it to stay consistent.
3. The keep index leaked between samples until a `begin_sample()` reset was added.

This is the substantive difference between hook-based and source-based instrumentation, and
it belongs in the paper. Patching the model file puts you inside the loop, where slicing
happens naturally. Working from outside means reconciling every tensor computed above the
loop.

### Not yet done

An InternVL2 adapter for the tool, roughly twenty lines: the decoder layer list, the image
token id, and how to build model inputs. Worth adding so the published tool covers both
